...
class RedisHandler:
    ...
    def publish(self, channel: str, msg: str, logger: Logger) -> None:
        # for SSE purpose, use channel as user_unique-id
        logger.debug(f"Redis publish channel: {channel} msg: {msg}")
        self.redis_client.publish(channel, msg)
    
    def subscribe(self, channel: str, logger: Logger) -> None:
        logger.debug(f"Redis subscribe channel: {channel}")
